...
void print_models(CommunicationProtocol& communication_protocol) {
  for (std::size_t id = 0; id <= kMaxServoId; id++) {
    auto _ = communication_protocol.read_model_number(id)
...
void ping(CommunicationProtocol& communication_protocol) {
  ...
  auto _ = communication_protocol.ping(id)
      .and_then([] {
...
void sync_read_position(CommunicationProtocol& communication_protocol) {
  ...
  while (true) {
    auto _ =  communication_protocol.sync_read(ids, SMS_STS_PRESENT_POSITION_L, &positions)
...
void sync_write_position(CommunicationProtocol& communication_protocol) {
  ...
  while (true) {
    ...
    auto _ =  communication_protocol.sync_write_position(ids, positions, speeds, accelerations)
        .or_else([=](const std::string& error) {
...
void reg_write_position(CommunicationProtocol& communication_protocol) {
  ...
  while (true) {
    ...
    for (uint8_t servo_id = 1; servo_id <= num_servos; servo_id++) {
      auto _ =  communication_protocol.reg_write_position(servo_id, data, 0, 0).or_else([=](const std::string& error) {
        throw std::runtime_error(fmt::format("Failed to set position [id={}][{}]", servo_id, error));
      });
    }
    get_input("Press enter to continue");
    auto _ =  communication_protocol.reg_write_action().or_else([](const std::string& error) {
...
void write_position(CommunicationProtocol& communication_protocol) {
  const auto id = std::stoi(get_input("Enter servo ID: "));
  auto _ =  communication_protocol.set_mode(id, OperationMode::kPosition).or_else([](const std::string& error) {
...
int main(int argc, char** argv) {
  ...
  auto _ = serial_port->configure().and_then([&] { return serial_port->open(); }).or_else([](const std::string& error) {
    throw std::runtime_error(error);
  });
